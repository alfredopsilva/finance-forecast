...
// ---------------------------------------------------------------------------
// 1.  Make the async handler generic so any promise type is accepted.
// ---------------------------------------------------------------------------
const asyncHandler =
  <T = unknown>(
    ...
    (req: Request, res: Response, next: NextFunction): void => {
      // We don’t care what `fn` returns; Express only needs us to swallow errors.
...
// ---------------------------------------------------------------------------
// 2.  Mark intentionally-unused parameters with a leading underscore.
// ---------------------------------------------------------------------------
  ...
  app.use(
    ...
    asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
      ...
      try {
        ...
        const entry: Record<string, any> | SSREntry = await (async () => {
          if (!isProd && vite) {
            // Development: Load module dynamically with Vite
            return await vite.ssrLoadModule("/src/entry-server.tsx");
          } else {
            // Production: Load pre-built module with dynamic path
            try {
              ...
              // Import with dynamic path
              const module = await import(entryPath);
              return module as SSREntry;
            } catch (error) {
              throw new SSRError(
                "Failed to load server entry. Make sure to run \"pnpm build\" first.",
                500,
              );
            }
          }
